...
pub const Registry = struct {
    /// The [error codes](https://connectrpc.com//docs/protocol/#error-codes) of the Connect request. Error codes are always string values.
    pub const connectRpcErrorCode = rpc_connect_rpc_error_code;
    /// Connect request metadata, `<key>` being the normalized Connect Metadata key (lowercase), the value being the metadata values.
    pub const connectRpcRequestMetadata = rpc_connect_rpc_request_metadata;
    /// Connect response metadata, `<key>` being the normalized Connect Metadata key (lowercase), the value being the metadata values.
    pub const connectRpcResponseMetadata = rpc_connect_rpc_response_metadata;
    /// The [numeric status code](https://github.com/grpc/grpc/blob/v1.33.2/doc/statuscodes.md) of the gRPC request.
    pub const grpcStatusCode = rpc_grpc_status_code;
    /// gRPC request metadata, `<key>` being the normalized gRPC Metadata key (lowercase), the value being the metadata values.
    pub const grpcRequestMetadata = rpc_grpc_request_metadata;
    /// gRPC response metadata, `<key>` being the normalized gRPC Metadata key (lowercase), the value being the metadata values.
    pub const grpcResponseMetadata = rpc_grpc_response_metadata;
    /// `error.code` property of response if it is an error response.
    pub const jsonrpcErrorCode = rpc_jsonrpc_error_code;
    /// `error.message` property of response if it is an error response.
    pub const jsonrpcErrorMessage = rpc_jsonrpc_error_message;
    /// `id` property of request or response. Since protocol allows id to be int, string, `null` or missing (for notifications), value is expected to be cast to string for simplicity. Use empty string in case of `null` value. Omit entirely if this is a notification.
    pub const jsonrpcRequestId = rpc_jsonrpc_request_id;
    /// Protocol version as in `jsonrpc` property of request/response. Since JSON-RPC 1.0 doesn't specify this, the value can be omitted.
    pub const jsonrpcVersion = rpc_jsonrpc_version;
    /// The name of the (logical) method being called, must be equal to the $method part in the span name.
    pub const method = rpc_method;
    /// The full (logical) name of the service being called, including its package name, if applicable.
    pub const service = rpc_service;
    /// A string identifying the remoting system. See below for a list of well-known identifiers.
    pub const system = rpc_system;
    /// Whether this is a received or sent message.
    pub const messageType = rpc_message_type;
    /// MUST be calculated as two different counters starting from `1` one for sent messages and one for received message.
    pub const messageId = rpc_message_id;
    /// Compressed size of the message in bytes.
    pub const messageCompressedSize = rpc_message_compressed_size;
    /// Uncompressed size of the message in bytes.
    pub const messageUncompressedSize = rpc_message_uncompressed_size;
};
